...
function mergeConnectionSources(...groups) {
  const out=[];
  ...
  for(const group of groups){
    for(const raw of Array.isArray(group)?group:[]){
      ...
      const fp=crypto.createHash('sha256').update(String(c.apiKey)).digest('hex');
      const existing=byFingerprint.get(fp);
      if(existing){
        existing.shopIds=[...new Set([...(existing.shopIds||[]),...(c.shopIds||[])].map(String))];
        existing.autoAllShops=!!(existing.autoAllShops||c.autoAllShops);
        continue;
      }
      const copy={...c,shopIds:[...(c.shopIds||[])].map(String)};
      byFingerprint.set(fp,copy);
      out.push(copy);
    }
  }
  return out;
...
export function readEnvSettings() {
  // v1.7.9: JSON and numbered env variables are additive instead of mutually
  // exclusive. This fixes the case where PANCAKE_CONNECTIONS_JSON already existed
  // and a newly-added PANCAKE_POS_API_KEY_4/_5 was silently ignored.
  const fromJson = envConnectionsFromJson();
  const numbered = numberedEnvConnections();
  const connections = mergeConnectionSources(fromJson, numbered);
  const source = fromJson.length && numbered.length ? 'env-json+env' : fromJson.length ? 'env-json' : numbered.length ? 'env' : 'none';
  return { source, shared: connections.length > 0, writable: false, connections };
}
...
export async function readSettingsAsync(headers = {}) {
  if (blobConfigEnabled()) {
    ...
    if (stored) {
      const connections=mergeConnectionSources(stored.connections,env.connections);
      return {...stored,source:env.connections.length?'vercel-blob+env':'vercel-blob',connections};
    }
...
function pancakeAccountName(json) {
  const root=asObject(json);
  const data=asObject(root.data);
  const buckets=[
    asObject(root.account),asObject(root.user),asObject(root.owner),asObject(root.profile),
    asObject(data.account),asObject(data.user),asObject(data.owner),asObject(data.profile)
  ];
  const direct=firstText(root.account_name,root.user_name,root.owner_name,root.profile_name,data.account_name,data.user_name,data.owner_name,data.profile_name);
  if(direct)return direct;
  for(const item of buckets){
    const name=firstText(item.name,item.display_name,item.full_name,item.username,item.email);
    if(name)return name;
  }
  return '';
}

function shopDirectoryRows(json) {
  const root=asObject(json);
  const data=asObject(root.data);
  const candidates=[root.shops,Array.isArray(root.data)?root.data:null,data.shops,root.items,data.items,root.results,data.results];
  for(const candidate of candidates)if(Array.isArray(candidate))return candidate;
  return [];
}

function shopDirectoryPageInfo(json) {
  const root=asObject(json),data=asObject(root.data),pagination=asObject(root.pagination),dataPagination=asObject(data.pagination);
  const totalPages=Math.max(0,Math.round(num(root.total_pages ?? root.totalPages ?? data.total_pages ?? data.totalPages ?? pagination.total_pages ?? pagination.totalPages ?? dataPagination.total_pages ?? dataPagination.totalPages ?? 0)));
  const totalEntries=Math.max(0,Math.round(num(root.total_entries ?? root.totalEntries ?? data.total_entries ?? data.totalEntries ?? pagination.total_entries ?? pagination.totalEntries ?? dataPagination.total_entries ?? dataPagination.totalEntries ?? 0)));
  return {totalPages,totalEntries};
}

async function fetchShopDirectoryRequest(apiKey,{page=null,pageSize=50}={}) {
  const url=new URL(`${BASE_URL}/shops`);
  url.searchParams.set('api_key',apiKey);
  if(Number.isInteger(page)&&page>0){
    // Pancake endpoints use both page/page_number conventions in different areas.
    // Sending the same value for both is harmless on gateways that ignore one of them.
...
export async function listShopsWithMeta(apiKey) {
  // First call keeps compatibility with Pancake's normal /shops response.
  const first=await fetchShopDirectoryRequest(apiKey);
  const firstRows=shopDirectoryRows(first);
  if(!firstRows.length)throw new Error('Connected, but this API key cannot access any stores');

  const byId=new Map();
  const addRows=rows=>{
    let added=0;
    for(const raw of rows){
      const x=asObject(raw);const id=String(x.id??x.shop_id??'').trim();if(!id)continue;
      const name=String(x.name??x.shop_name??x.title??id??'Shop');
      if(!byId.has(id)){byId.set(id,{id,name});added++;}
      else if((!byId.get(id).name||byId.get(id).name===id)&&name)byId.get(id).name=name;
    }
    return added;
  };
  addRows(firstRows);

  const firstInfo=shopDirectoryPageInfo(first);
  let pagesFetched=1;
  let paginated=false;
  // The legacy directory has been observed returning around 50 rows without an
  // explicit pagination envelope. Probe paged mode whenever the first response is
  // large enough to plausibly be capped, or when Pancake explicitly reports >1 page.
  const shouldProbe=firstRows.length>=50||firstInfo.totalPages>1||(firstInfo.totalEntries>firstRows.length&&firstInfo.totalEntries>0);
  if(shouldProbe){
    paginated=true;
    const PAGE_SIZE=50,MAX_PAGES=20;
    let knownTotalPages=firstInfo.totalPages;
    for(let page=1;page<=MAX_PAGES;page++){
      let j;
      try{j=await fetchShopDirectoryRequest(apiKey,{page,pageSize:PAGE_SIZE});}
      catch(e){
        // If this Pancake gateway rejects paging params, keep the valid first response
        // instead of turning a working account into an error.
        if(page===1)break;
        throw e;
      }
      pagesFetched++;
      const rows=shopDirectoryRows(j);
      const info=shopDirectoryPageInfo(j);
      if(info.totalPages)knownTotalPages=info.totalPages;
      const added=addRows(rows);
      if(!rows.length)break;
      if(knownTotalPages&&page>=knownTotalPages)break;
      // Always allow page 1 to duplicate the compatibility call; page 2 tells us
      // whether pagination is real. Afterwards, no new IDs means the params are ignored.
      if(page>=2&&added===0)break;
      if(page>=2&&rows.length<PAGE_SIZE&&!knownTotalPages)break;
    }
  }

  const shops=[...byId.values()].sort((a,b)=>String(a.name).localeCompare(String(b.name),'th')||String(a.id).localeCompare(String(b.id)));
  return {shops,accountName:pancakeAccountName(first),pagesFetched,paginated,firstPageCount:firstRows.length,reportedTotal:firstInfo.totalEntries||shops.length};
}

export async function listShops(apiKey) {
  return (await listShopsWithMeta(apiKey)).shops;
}

// v1.8.0: Facebook Page discovery is intentionally separate from POS Shop
// discovery. /shops returns POS Shop IDs used by Employee Statistic totals,
// while pages.fm /pages returns actual Facebook Page IDs visible to a Pancake
// User Access Token. One POS Shop can contain multiple Facebook Pages.
const PANCAKE_USER_API = 'https://pages.fm/api/v1';
const PAGE_FETCH_TIMEOUT_MS = 12000;
const NEGATIVE_PAGE_BUCKET = /inactive|disabled|deactivated|expired|removed|revoked|unavailable|blocked|deleted|archived|suspended/i;

function normalizeUserTokenEntry(raw, i = 0) {
  if (typeof raw === 'string') return { id:`user-${i+1}`, label:`Pancake User ${i+1}`, token:String(raw).trim() };
  const x = asObject(raw);
  return {
    id:String(x.id || `user-${i+1}`).slice(0,40),
    label:String(x.label || x.name || `Pancake User ${i+1}`).slice(0,80),
    token:String(x.accessToken || x.access_token || x.token || '').trim()
  };
}

function userTokenConnectionsFromJson() {
  const raw = String(process.env.PANCAKE_USER_TOKENS_JSON || process.env.PANCAKE_USER_ACCESS_TOKENS_JSON || '').trim();
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed.map(normalizeUserTokenEntry).filter(x=>x.token);
  } catch { return []; }
}

function numberedUserTokenConnections() {
  const slots = new Map();
  const base = String(process.env.PANCAKE_USER_ACCESS_TOKEN || '').trim();
  if (base) slots.set('0',{token:base,label:String(process.env.PANCAKE_USER_LABEL || process.env.PANCAKE_USER_LABEL_1 || '').trim(),id:'user-env-base'});
  for (const key of Object.keys(process.env)) {
    const m = key.match(/^PANCAKE_USER_ACCESS_TOKEN_(.+)$/);
    if (!m) continue;
    const suffix=m[1];
    if (base && suffix==='1') continue;
    const token=String(process.env[key]||'').trim();
    if (!token) continue;
    slots.set(suffix,{token,label:String(process.env[`PANCAKE_USER_LABEL_${suffix}`]||'').trim(),id:`user-env-${suffix}`});
  }
  const natural=(a,b)=>{
    const an=/^\d+$/.test(a),bn=/^\d+$/.test(b);
    if(an&&bn)return Number(a)-Number(b);if(an)return -1;if(bn)return 1;
    return a.localeCompare(b,undefined,{numeric:true,sensitivity:'base'});
  };
  return [...slots.entries()].sort((a,b)=>natural(a[0],b[0])).map(([_,x],i)=>normalizeUserTokenEntry({id:x.id,label:x.label||`Pancake User ${i+1}`,accessToken:x.token},i));
}

function readUserTokenConnections() {
  const out=[];const seen=new Set();
  for(const x of [...userTokenConnectionsFromJson(),...numberedUserTokenConnections()]){
    if(!x.token)continue;const fp=crypto.createHash('sha256').update(x.token).digest('hex');
    if(seen.has(fp))continue;seen.add(fp);out.push(x);
  }
  return out;
}

function visiblePageRows(raw) {
  const root=asObject(raw),data=asObject(root.data);
  const categorized=root.categorized || data.categorized;
  if(categorized && typeof categorized==='object' && !Array.isArray(categorized)){
    return Object.values(categorized).filter(Array.isArray).flatMap(x=>x);
  }
  const candidates=[Array.isArray(raw)?raw:null,root.pages,data.pages,Array.isArray(root.data)?root.data:null];
  for(const c of candidates)if(Array.isArray(c))return c;
  return [];
}

function pageIdOf(raw){const x=asObject(raw);return String(x.id??x.page_id??'').trim();}
function pageLooksUsable(raw){
  const x=asObject(raw);
  if(x.active===false||x.activated===false||x.enabled===false||x.is_active===false||x.is_activated===false||x.disabled===true||x.is_disabled===true||x.deleted===true||x.is_deleted===true)return false;
  const state=String(x.status||x.state||x.category||x.page_status||x.connection_status||'').toLowerCase();
  return !NEGATIVE_PAGE_BUCKET.test(state);
}
function dedupePages(rows){
  const out=[];const seen=new Set();
  for(const raw of rows||[]){const id=pageIdOf(raw);if(!id||seen.has(id))continue;seen.add(id);const x=asObject(raw);out.push({id,name:firstText(x.name,x.page_name,x.title,id)});}
  return out;
}
function normalizeVisibleFacebookPages(raw){return dedupePages(visiblePageRows(raw));}
function normalizeActiveFacebookPages(raw){
  const root=asObject(raw),data=asObject(root.data);const categorized=root.categorized||data.categorized;
  if(categorized&&typeof categorized==='object'&&!Array.isArray(categorized)){
    const rows=Object.entries(categorized).filter(([k,v])=>Array.isArray(v)&&!NEGATIVE_PAGE_BUCKET.test(String(k))).flatMap(([,v])=>v).filter(pageLooksUsable);
    return dedupePages(rows);
  }
  return dedupePages(visiblePageRows(raw).filter(pageLooksUsable));
}

async function fetchFacebookPages(accessToken){
  const url=new URL(`${PANCAKE_USER_API}/pages`);url.searchParams.set('access_token',accessToken);
  let lastError=null;
  for(let attempt=1;attempt<=2;attempt++){
    try{
      const r=await fetch(url,{cache:'no-store',signal:AbortSignal.timeout(PAGE_FETCH_TIMEOUT_MS),headers:{accept:'application/json'}});
      const text=await r.text().catch(()=> '');let j={};try{j=text?JSON.parse(text):{}}catch{j={}}
      if(!r.ok||j?.success===false){const detail=extractErrorDetail(j)||(text&&text.length<180?text:'');const err=new Error(detail?`Pancake Pages API ${r.status} · ${detail}`:`Pancake Pages API ${r.status}`);if((r.status===429||r.status>=500)&&attempt<2){lastError=err;await sleep(NETWORK_RETRY_DELAY_MS*attempt);continue}throw err;}
      return j;
    }catch(e){lastError=e;if(attempt<2&&isTimeoutError(e)){await sleep(NETWORK_RETRY_DELAY_MS*attempt);continue}throw e;}
  }
  throw lastError||new Error('Unable to load Facebook Pages');
}

export async function discoverFacebookPages() {
  const credentials=readUserTokenConnections();
  if(!credentials.length)return {configured:false,accessTokens:0,visibleBeforeDedupe:0,activeBeforeDedupe:0,activeUniquePages:0,duplicatesCollapsed:0,filteredPages:0,pages:[],credentials:[],errors:[]};
  const byPage=new Map();const errors=[];const stats=[];let totalVisible=0,totalActive=0;
  for(const cred of credentials){
    try{
      const raw=await fetchFacebookPages(cred.token);const visible=normalizeVisibleFacebookPages(raw);const active=normalizeActiveFacebookPages(raw);
      totalVisible+=visible.length;totalActive+=active.length;stats.push({label:cred.label,visiblePages:visible.length,activePages:active.length,filteredPages:Math.max(0,visible.length-active.length)});
      for(const p of active){
        if(!byPage.has(p.id))byPage.set(p.id,{...p,accounts:[cred.label]});
        else if(!byPage.get(p.id).accounts.includes(cred.label))byPage.get(p.id).accounts.push(cred.label);
      }
    }catch(e){errors.push({label:cred.label,error:e?.message||String(e)});stats.push({label:cred.label,visiblePages:0,activePages:0,filteredPages:0,error:e?.message||String(e)});}
  }
  const pages=[...byPage.values()].sort((a,b)=>String(a.name).localeCompare(String(b.name),'th')||String(a.id).localeCompare(String(b.id)));
  return {configured:true,accessTokens:credentials.length,visibleBeforeDedupe:totalVisible,activeBeforeDedupe:totalActive,activeUniquePages:pages.length,duplicatesCollapsed:Math.max(0,totalActive-pages.length),filteredPages:Math.max(0,totalVisible-totalActive),pages,credentials:stats,errors};
}
...
async function listShopsCached(apiKey) {
  ...
  const directory = await listShopsWithMeta(apiKey);
  shopCache.set(cacheKey, { ...directory, expiresAt: Date.now() + SHOP_CACHE_TTL_MS });
  return directory.shops;
}
...
function summaryDiscountEvidence(json) {
  const summary=employeeSummaryObject(json);
  const raw={};
  const bahtGuess={};
  for(const [key,value] of Object.entries(summary)){
    if(!/(discount|coupon|voucher|promo|promotion|shipping|cod|net|after_sub)/i.test(String(key)))continue;
    if(typeof value==='number'||(typeof value==='string'&&value.trim()!==''&&Number.isFinite(Number(value.replace(/,/g,''))))){
      raw[key]=value;
      bahtGuess[key]=money(value);
    }
  }
  return {raw,bahtGuess};
}
...
const PAGE_HEALTH_BATCH_MAX = 6;
const PAGE_HEALTH_TIMEOUT_MS = 4800;

function pageHealthFailure(error) {
  const message = String(error?.message || error || 'Pancake connection failed');
  if (/429|rate limit/i.test(message)) return { code:'RATE_LIMIT', label:'RATE LIMIT', message };
  if (/timeout|timed out|aborted/i.test(message)) return { code:'TIMEOUT', label:'TIMEOUT', message };
  if (/403|forbidden|permission|no permission|access denied|not allowed/i.test(message)) return { code:'NO_ACCESS', label:'NO ACCESS', message };
  if (/401|unauthor|invalid.*key|api key/i.test(message)) return { code:'AUTH', label:'API KEY', message };
  if (/404|not found|does not exist/i.test(message)) return { code:'NOT_FOUND', label:'NOT FOUND', message };
  if (/authoritative|summary\.price|report shape|employee statistic/i.test(message)) return { code:'REPORT_INVALID', label:'REPORT INVALID', message };
  return { code:'API_ERROR', label:'API ERROR', message };
}

// Checks whether one configured Pancake credential can actually read the same
// Employee Statistic endpoint used by the live total. The browser sends small
// batches so 100+ shops never have to fit inside one Vercel invocation.
export async function checkPageHealth(headers = {}, body = {}) {
  const settings = await readSettingsAsync(headers);
  const connectionId = String(body?.connectionId || '').trim();
  const connection = settings.connections.find(c => String(c.id) === connectionId) || null;
  const apiKey = String(body?.apiKey || connection?.apiKey || '').trim();
  const label = String(connection?.label || body?.label || 'Pancake API');
  if (!apiKey) throw new Error('Pancake API key is missing for this connection');

  const shopIds = [...new Set((Array.isArray(body?.shopIds) ? body.shopIds : [])
    .map(x => String(x || '').trim()).filter(Boolean))];
  if (!shopIds.length) return {ok:true,connectionId,label,results:[],checked:0};
  if (shopIds.length > PAGE_HEALTH_BATCH_MAX) throw new Error(`Page health batch is limited to ${PAGE_HEALTH_BATCH_MAX} shops`);

  const today = dateKeyBangkok();
  const since = localRangeValue(today, false);
  const until = stableLiveCutoff(Date.now()).until;
  const settled = await allSettledLimit(shopIds, Math.min(PAGE_HEALTH_BATCH_MAX, shopIds.length), async shopId => {
    const raw = await fetchAnalyticsSale(apiKey, shopId, since, until, {
      splitFields:['User.id'],
      timeoutMs:PAGE_HEALTH_TIMEOUT_MS
    });
    const parsed = parsePancakeSalesSummary(raw);
    if (!parsed) throw new Error(`Employee Statistic has no authoritative summary.price (${responseShape(raw)})`);
    return {
      shopId,
      ok:true,
      code:'OK',
  ...
  const results = settled.map((result,index) => {
    ...
    const fail = pageHealthFailure(result.reason);
    return {shopId,ok:false,...fail};
  });
  return {
    ...
    connected:results.filter(x=>x.ok).length,
    failed:results.filter(x=>!x.ok).length,
    results,
    observedThrough:until,
    source:'Pancake Employee Statistic /analytics/sale access check'
  };
}
...
function orderItemsFromRow(row) {
  ...
  const candidates=[r.items,r.order_items,r.line_items,r.products,data.items,data.order_items];
  let rows=[];
  for(const c of candidates){if(Array.isArray(c)){rows=c;break}}
  return rows.map(raw=>{
    const item=asObject(raw);
    const variation=asObject(item.variation_info ?? item.variation ?? item.actual_variation_info);
    const product=asObject(item.product_info ?? item.product);
    const name=firstText(
      item.product_name,item.name,item.display_name,item.title,item.variation_name,
      variation.name,variation.product_name,variation.title,
      product.name,product.product_name,product.title
    );
    const code=firstText(
      item.sku,item.product_code,item.code,item.custom_id,
      variation.custom_id,variation.sku,variation.code,variation.barcode,variation.product_id,
      product.custom_id,product.sku,product.code,product.barcode
    );
    const productId=firstText(item.product_id,product.id,variation.product_id);
    const variationId=firstText(item.variation_id,variation.id);
    const quantity=Math.max(1,Math.round(num(item.quantity ?? item.qty ?? item.count ?? 1))||1);
    return {name,code,productId,variationId,quantity};
  }).filter(x=>x.name||x.code||x.productId||x.variationId);
}

function pancakeOrderTimestampMs(value,referenceMs=Date.now()) {
  const raw=String(value??'').trim();
  if(!raw)return NaN;
  const normalized=raw.replace(' ','T');
  if(/[zZ]|[+-]\d\d:?\d\d$/.test(normalized))return Date.parse(normalized);
  const utc=Date.parse(`${normalized}Z`);
  const bangkok=Date.parse(`${normalized}+07:00`);
  const candidates=[utc,bangkok].filter(Number.isFinite);
...
function orderEventFromRow(row,shopId) {
  const r=asObject(row);
  const hasAfterDiscount=r.total_price_after_sub_discount!==undefined&&r.total_price_after_sub_discount!==null;
  const hasTotal=r.total_price!==undefined&&r.total_price!==null;
  if(!hasAfterDiscount&&!hasTotal) return null;
  const id=String(r.id ?? r.order_id ?? r.display_id ?? '').trim();
  if(!id) return null;
  // Employee Statistic revenue is a net sales metric. For per-order display/reconciliation
  // prefer Pancake's explicit post-discount amount when present. Never invent a discount
  // formula; older responses without that field fall back to total_price.
  const rawPrice=num(hasAfterDiscount?r.total_price_after_sub_discount:r.total_price);
  const shop=asObject(r.shop);
  const page=asObject(r.page);
  const account=asObject(r.account);
  return {
    id,
    orderCode:firstText(r.display_id,r.custom_id,r.order_code,r.code,id),
    shopId:String(shopId),
    shopName:firstText(r.page_name,r.shop_name,page.name,page.page_name,page.title,shop.name,shop.shop_name),
    amount:money(rawPrice),
    rawPrice,
    amountSource:hasAfterDiscount?'total_price_after_sub_discount':'total_price',
    totalDiscount:money(r.total_discount??0),
    insertedAt:String(r.inserted_at ?? r.created_at ?? ''),
    accountName:firstText(r.account_name,account.name,account.display_name,account.full_name),
    items:orderItemsFromRow(r)
  };
}
...
async function fetchOrderEventsRange(apiKey,shopId,sinceIso,untilIso) {
  ...
  // Keep a small real-order lookback for the display feed because Pancake's Employee
  // Statistic and /orders endpoints can surface the same sale a few seconds apart.
  // Strict score reconciliation still filters to the exact snapshot interval below.
  const feedLookbackMs=20_000;
  url.searchParams.set('startDateTime',String(Math.floor((sinceMs-feedLookbackMs)/1000)));
  ...
  for(let attempt=1;attempt<=2;attempt++){
    try{
      ...
      const allEvents=rows.map(row=>orderEventFromRow(row,shopId)).filter(Boolean).map(event=>({
        ...event,
        insertedAtMs:pancakeOrderTimestampMs(event.insertedAt,untilMs)
      })).filter(event=>Number.isFinite(Number(event.insertedAtMs)));
      const exactEvents=allEvents.filter(event=>Number(event.insertedAtMs)>sinceMs&&Number(event.insertedAtMs)<=untilMs+999);
      const feedEvents=allEvents.filter(event=>Number(event.insertedAtMs)>sinceMs-feedLookbackMs&&Number(event.insertedAtMs)<=untilMs+999);
      if(totalPages>1) return {complete:false,events:[],feedEvents,reason:`More than ${ORDER_EVENT_PAGE_SIZE} orders exist in the lookup window`};
      return {complete:true,events:exactEvents,feedEvents};
    }catch(e){
      lastError=e;
      if(attempt<2&&isTimeoutError(e)){await sleep(NETWORK_RETRY_DELAY_MS*attempt);continue}
      throw e;
...
async function resolveConnectionShops(connection) {
  // Manual Shop IDs must not depend on the /shops directory being available.
  // Their display name is resolved lazily only when a changed shop needs an order lookup.
  if (connection.shopIds.length) return connection.shopIds.map(id=>({id:String(id),name:''}));
...
async function uniqueJobsFromSettings(settings) {
  ...
  const discovered = await Promise.allSettled(connections.map(async (c,connectionIndex)=>({c,connectionIndex,shops:await resolveConnectionShops(c)})));
  discovered.forEach((result,index)=>{
    ...
    for(const shop of result.value.shops){
      const id=String(shop?.id||'');
      if(!id)continue;
      const name=String(shop?.name||'');
      const credential={apiKey:result.value.c.apiKey,label:result.value.c.label,connectionId:result.value.c.id,connectionIndex:result.value.connectionIndex};
      if(!unique.has(id)) unique.set(id,{shopId:id,shopName:name,credentials:[credential]});
      else {
        if(!unique.get(id).shopName&&name)unique.get(id).shopName=name;
        if(!unique.get(id).credentials.some(x=>x.apiKey===credential.apiKey)) unique.get(id).credentials.push(credential);
      }
    }
  });
...
function publicPlanJob(job) {
  return [String(job.shopId),(job.credentials||[]).map(c=>Number(c.connectionIndex)).filter(Number.isInteger),String(job.shopName||'')];
}
...
export async function fetchReportBatch(headers={},body={}) {
  ...
  settled.forEach((r,i)=>{
    ...
    if(plan.kind==='live'){
      ...
      results.push({
        index,shopId:job.shopId,shopName:job.shopName||'',ok:true,label:used.label,
        revenue:value.revenue,orders:value.orders,products:value.products,
        metricKey:value.metricKey,zeroSales:!!value.zeroSales
      });
    }else{
      results.push({index,shopId:job.shopId,shopName:job.shopName||'',ok:true,label:used.label,days:r.value.value});
    }
  });
...
export async function fetchVerifiedOrderEvents(headers={},body={}) {
  ...
  if(currentMs-previousMs>120000) return {complete:false,events:[],feedEvents:[],reason:'Snapshot gap is too large for per-order animation'};

  const requested=[...new Set((Array.isArray(body?.shopIds)?body.shopIds:[]).map(String).filter(Boolean))];
  if(!requested.length) return {complete:true,events:[],feedEvents:[],shops:0};
  if(requested.length>ORDER_EVENT_MAX_SHOPS) return {complete:false,events:[],feedEvents:[],reason:`Too many changed shops for order lookup (${requested.length})`};
  ...
  const settled=await allSettledLimit(selected,Math.min(6,selected.length),job=>fetchWithCredentialFallbackBounded(job,async c=>{
    const value=await fetchOrderEventsRange(c.apiKey,job.shopId,previousObservedThrough,plan.until);
    let resolvedShopName=job.shopName||'';
    const rows=[...(value.events||[]),...(value.feedEvents||[])];
  ...
  const events=[]; const displayEvents=[]; const errors=[];
  settled.forEach((r,i)=>{
    const job=selected[i];
    if(r.status==='rejected'){
    ...
    const used=r.value.credential;
    const enrich=ev=>({...ev,shopName:ev.shopName||r.value.value?.resolvedShopName||job.shopName||'',apiLabel:used.label,connectionId:String(used.connectionId||''),accountName:ev.accountName||''});
    for(const ev of r.value.value?.feedEvents||r.value.value?.events||[]) displayEvents.push(enrich(ev));
    if(r.value.value?.complete!==true){
      errors.push(`${used.label}/${job.shopId}: ${r.value.value?.reason||'order-event interval incomplete'}`);
      return;
    }
    for(const ev of r.value.value.events||[]) events.push(enrich(ev));
  });
  const eventMs=ev=>Number(ev.insertedAtMs)||pancakeOrderTimestampMs(ev.insertedAt,currentMs);
  events.sort((a,b)=>eventMs(a)-eventMs(b)||String(a.id).localeCompare(String(b.id)));
  displayEvents.sort((a,b)=>eventMs(a)-eventMs(b)||String(a.id).localeCompare(String(b.id)));
  const dedupe=list=>{const seen=new Set();return list.filter(ev=>{const key=`${String(ev.shopId)}:${String(ev.id)}`;if(seen.has(key))return false;seen.add(key);return true;});};
  const feedEvents=dedupe(displayEvents);
  const strictEvents=dedupe(events);
  return {
    complete:errors.length===0,
    // Only the exact, fully complete interval may drive individual score hits.
    events:errors.length?[]:strictEvents,
    // feedEvents may use the small real-order lookback, but are display evidence only.
    // They never alter or split the authoritative Employee Statistic total.
    feedEvents,
    shops:selected.length,
    previousObservedThrough,
    observedThrough:plan.until,
    errors,
    source:'Pancake /orders inserted_at · latest-order display evidence; Employee Statistic remains total source of truth'
  };
}
  ...
  for(const job of sampleJobs){
    try{
      ...
      samples.push({
        ...
        discountEvidence:summaryDiscountEvidence(result.value),
        summaryKeys:Object.keys(employeeSummaryObject(result.value)).slice(0,40),
        orderAmountPolicy:'total_price_after_sub_discount when present; otherwise total_price',
