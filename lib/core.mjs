...
function orderEventFromRow(row,shopId) {
  ...
  return {
    ...
    shopName:firstText(r.shop_name,r.page_name,shop.name,shop.shop_name),
...
async function fetchOrderEventsRange(apiKey,shopId,sinceIso,untilIso) {
  ...
  for(let attempt=1;attempt<=2;attempt++){
    try{
      ...
      const events=rows.map(row=>orderEventFromRow(row,shopId)).filter(Boolean).map(event=>({
        ...event,
...
export async function fetchVerifiedOrderEvents(headers={},body={}) {
  ...
  settled.forEach((r,i)=>{
    ...
    for(const ev of r.value.value.events||[]) events.push({...ev,apiLabel:r.value.credential.label,connectionId:String(r.value.credential.connectionId||''),accountName:ev.accountName||''});
  });
  events.sort((a,b)=>(Number(a.insertedAtMs)||pancakeOrderTimestampMs(a.insertedAt,currentMs))-(Number(b.insertedAtMs)||pancakeOrderTimestampMs(b.insertedAt,currentMs))||String(a.id).localeCompare(String(b.id)));
  return {
