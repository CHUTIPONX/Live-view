...
async function fetchStatistics(apiKey, shopId, startDate, endDate, groupBy = '') {
  ...
  if (!r.ok || j?.success === false) {
    ...
    const detail = String(root.message ?? root.error ?? data.message ?? data.error ?? '').trim();
    const base = r.status === 429 ? '429 rate limit' : `API ${r.status}`;
    throw new Error(detail ? `${base} · ${detail}` : `${base} · Pancake returned success=false`);
  }
  return j;
}
...
async function fetchHistoryFallback(apiKey, shopId, days) {
  return Promise.all(days.map(async day => {
    ...
    return { date: day, revenue: summary.revenue, orders: summary.orders };
  }));
}
...
function uniqueJobsFromSettings(settings) {
  return (async () => {
...
export async function aggregateSales(headers = {}) {
  ...
  if (!jobs.length) return {
    ...
    source: 'Pancake statistics live summary',
    moneyUnit: 'baht'
  };

  // IMPORTANT: /api/sales is the 1-second live endpoint.
  // Do NOT fetch multi-day history here. History has its own endpoint so this request
  // stays under Vercel's function timeout and the dashboard never goes blank.
  ...
  currentResults.forEach((r, i) => {
    if (r.status === 'rejected') {
      errors.push(`${jobs[i].label}/${jobs[i].shopId}: ${r.reason?.message || 'sales error'}`);
      return;
    }
  ...
  const list = days.map(date => ({ date, revenue: 0, orders: 0 }));
  list[list.length - 1] = { date: days.at(-1), revenue: total, orders };
...
export async function aggregateHistory(headers = {}) {
  const settings = readSettings(headers);
  const { jobs, discoveryErrors } = await uniqueJobsFromSettings(settings);
  const allDays = fiveDays();
  const historyDays = allDays.slice(0, -1);
  ...
  const results = await allSettledLimit(jobs, 4, j => fetchHistoricalStats(j.apiKey, j.shopId));
  const merged = new Map(historyDays.map(date => [date, {date,revenue:0,orders:0}]));
  const errors = [...discoveryErrors];

  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      errors.push(`${jobs[i].label}/${jobs[i].shopId}: ${r.reason?.message || 'history error'}`);
      return;
    }
  ...
  return {
    days: historyDays.map(d => merged.get(d)),
    updatedAt: new Date().toISOString(),
