import { aggregateHistory, aggregateSales, blobConfigEnabled, checkPageHealth, clearCsrfCookie, clearSessionCookie, createReportPlan, csrfCookie, csrfToken, diagnoseSales, discoverFacebookPages, fetchReportBatch, fetchVerifiedOrderEvents, isAuthed, isCsrfValid, listShopsWithMeta, publicSettingsAsync, readSettingsAsync, sessionCookie, settingsCookie, writeBlobSettings } from './core.mjs';
...
export async function shops({headers,body}) {
  ...
  try { const directory=await listShopsWithMeta(key); return json(200,{ok:true,shops:directory.shops,shopCount:directory.shops.length,pagesFetched:directory.pagesFetched||1,paginated:!!directory.paginated,firstPageCount:directory.firstPageCount||directory.shops.length,reportedTotal:directory.reportedTotal||directory.shops.length,accountName:directory.accountName||'',accountNameSource:directory.accountName?'pancake-response':'configured-label'}); } catch(e) { return json(502,{ok:false,error:e?.message||'Connection failed'}); }
}

export async function facebookPages({headers}) {
  if (!isAuthed(headers)) return json(401,{error:'Unauthorized'});
  try { return json(200,{ok:true,...await discoverFacebookPages()}); }
  catch(e) { return json(502,{ok:false,error:e?.message||'Facebook Page discovery failed'}); }
}

export async function pageHealth({headers,body={}}) {
  if (!isAuthed(headers)) return json(401,{error:'Unauthorized'});
  const blocked = csrfOr403(headers); if (blocked) return blocked;
  try { return json(200, await checkPageHealth(headers, body)); }
  catch(e) { return json(502,{ok:false,error:e?.message||'Page connection check failed'}); }
}
