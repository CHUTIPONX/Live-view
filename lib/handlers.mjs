...
export async function shops({headers,body}) {
  ...
  try { const directory=await listShopsWithMeta(key); return json(200,{ok:true,shops:directory.shops,accountName:directory.accountName||'',accountNameSource:directory.accountName?'pancake-response':'configured-label'}); } catch(e) { return json(502,{ok:false,error:e?.message||'Connection failed'}); }
}
