...
const api = {
  '/api/login':login,
  '/api/logout':logout,
  '/api/security-token':securityToken,
...
function send(res,out){res.writeHead(out.status,{...SECURITY_HEADERS,...(out.headers||{})});res.end(out.body||'');}
...
const server=http.createServer(async(req,res)=>{
  ...
  try{const data=await fs.readFile(file);res.writeHead(200,{...SECURITY_HEADERS,'content-type':mime[path.extname(file)]||'application/octet-stream','cache-control':'private, no-cache'});res.end(data)}catch{res.writeHead(404,SECURITY_HEADERS);res.end('Not found')}
});
