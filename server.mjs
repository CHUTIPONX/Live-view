...
import { getSettings, login, logout, sales, saveSettings, shops } from './lib/handlers.mjs';

const root = path.dirname(fileURLToPath(import.meta.url));
const pub = path.join(root,'public');
const port = Number(process.env.PORT||3000);
const api = { '/api/login':login, '/api/logout':logout, '/api/settings:get':getSettings, '/api/settings:post':saveSettings, '/api/shops':shops, '/api/sales':sales };
