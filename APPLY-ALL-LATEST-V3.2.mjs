...
console.log('\n=== LIVE-VIEW ALL-IN-ONE LATEST V3.2 ===');
...
console.log('Step 3/4 · Forcing LIVE ORDERS to the bottom on every screen size...');
const cssPath=path.join(root,'public','style.css');
let css=fs.readFileSync(cssPath,'utf8');
const bottomMarker='/* v3.2 — LIVE ORDERS ALWAYS BOTTOM */';
if(!css.includes(bottomMarker)){
  css += `
...
@media(max-width:700px){
  .sales-layout{gap:14px!important}
  .live-order-list{
    display:flex!important;
    flex-wrap:nowrap!important;
    overflow-x:auto!important;
...
console.log('Step 4/4 · Final syntax + npm test...');
...
if(fs.existsSync(path.dirname(exclude))){
  ...
  for(const line of ['APPLY-ALL-LATEST-V3.2.mjs','_live_view_v3_support/']){
...
console.log('\n==============================================');
console.log('ALL-IN-ONE LATEST V3.2: PASS');
...
console.log('✓ LIVE ORDERS always bottom on every screen size');
